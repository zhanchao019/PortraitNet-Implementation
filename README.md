...
----
## Notice
There are still bugs in implenting the model restore process, the result in the report is generated by testing script.

the multi gpu training is implented in parallel branch with ACCELERATE Framework. However, due to the time limit, the whole project is still need to fix.
...
import torch.backends.cudnn as cudnnx
...
def test(dataLoader, netmodel, optimizer, epoch, logger, exp_args):
    ...
    for i, (input_ori, input, edge, mask) in enumerate(dataLoader):  
        ...
        if exp_args.addEdge == True:
            ...
            loss_mask = loss_Softmax(output_mask, mask_var)
            losses_mask.update(loss_mask.data.data, input.size(0))
            loss = loss_mask
        ...
        # total loss
        loss = loss_mask
        losses.update(loss.data.data, input.size(0))
...
def main(args):
    ...
    if exp_args.init == True:
        ...
        pretrained_state_dict_keys=list(pretrained_state_dict_keys)#用来兼容
        netmodel_state_dict_keys=list(netmodel_state_dict_keys)#用来兼容
        ...
        for k in range(len(pretrained_state_dict_keys)):
            #print(pretrained_state_dict.__type__)#order_dict
